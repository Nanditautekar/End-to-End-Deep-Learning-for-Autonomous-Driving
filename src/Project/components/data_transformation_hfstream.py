...
def paths_exist(row):
    for k in IMAGE_KEYS:
        if k not in row or row[k] is None:
            return False
        if not os.path.exists(resolve_path(row[k])):
            return False
    return True
...
def transform_to_json():
    try:
        ...
        logging.info(f"Total raw rows: {len(data)}")
        ...
        logging.info(f"Valid rows kept: {len(valid_rows)}")
        logging.info(f"Skipped (missing images): {skipped_missing}")
        logging.info(f"Skipped (invalid boxes): {skipped_boxes}")
        ...
        # TRAIN-ONLY FILTERING (BORING STRAIGHT SAMPLES)
        ...
        for row in train_data:
            ...
            is_boring = abs(steer) < STEER_EPS and brake <= BRAKE_EPS

            if is_boring and random.random() < DROP_PROB:
                dropped += 1
                continue

            filtered_train.append(row)

        logging.info(f"Dropped straight/no-brake train samples: {dropped}")
        logging.info(f"Final train samples: {len(filtered_train)}")
        ...
        logging.info("JSON transformation completed successfully")
        logging.info(f"Train: {len(filtered_train)} | Val: {len(val_data)} | Test: {len(test_data)}")
